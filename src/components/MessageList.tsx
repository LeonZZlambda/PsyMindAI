...
const ImageViewer: React.FC<{ src: string; alt: string; onClose: () => void }> = ({
  src,
  alt,
  onClose,
}) => {
      ...
      <div className="viewer-header">
        <div className="viewer-actions">
          <button
            ...
            onClick={handleCopy}
            title={copied ? t('chat.messages.copied_short') : t('chat.messages.copy_image')}
          >
            <span className="material-symbols-outlined">{copied ? 'check' : 'content_copy'}</span>
          </button>
...
const MessageList: React.FC = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([import('prismjs'), import('dompurify')])
    ...
    <main className="chat-container" ref={chatContainerRef} onScroll={handleScroll}>
      {messages.length === 0 ? (
                ...
                {isLoadingQuote ? (
                  ...
                  <>
                    <div className="daily-quote-container">
                      <span className="material-symbols-outlined quote-icon quote-start">
                        format_quote
                      </span>
                      <p className="daily-quote">{dailyQuote}</p>
                      <span className="material-symbols-outlined quote-icon quote-end">
                        format_quote
                      </span>
                    </div>
          ...
          {isAnonymous ? (
            <div className="anonymous-disclaimer">
              <div className="anonymous-notice-container">
                <span className="material-symbols-outlined anonymous-notice-icon">
                  visibility_off
                </span>
              </div>
              <h3 className="anonymous-notice-title">
                {t('chat.messages.anonymous_private_mode_title')}
              </h3>
              <p className="anonymous-notice-desc">
                <strong>{t('chat.messages.anonymous_desc_1')}</strong>
              </p>
              <p className="anonymous-notice-sub">{t('chat.messages.anonymous_desc_2')}</p>
            </div>
          ) : (
            <div
              className="suggestions"
              role="group"
              aria-label={t('chat.messages.suggestions_aria')}
            >
              <button
              ...
              >
                <strong>
                  ...
                  {t('chat.messages.suggestions.anxiety_title')}
                </strong>
                <br />
              ...
              >
                <strong>
                  ...
                  {t('chat.messages.suggestions.focus_title')}
                </strong>
                <br />
              ...
              >
                <strong>
                  ...
                  {t('chat.messages.suggestions.esteem_title')}
                </strong>
                <br />
              ...
              >
                <strong>
                  ...
                  {t('chat.messages.suggestions.techniques_title')}
                </strong>
                <br />
              ...
              >
                <strong>
                  ...
                  {t('chat.messages.suggestions.schedule_title')}
                </strong>
                <br />
              ...
              >
                <strong>
                  ...
                  {t('chat.messages.suggestions.burnout_title')}
                </strong>
                <br />
        ...
        <div className="messages">
                ...
                <div className="message-content-wrapper">
                        ...
                        code({ node, inline, className, children, ...props }: any) {
                          ...
                          if (!inline && match && prismRef.current) {
                            ...
                            const highlighted = grammar
                              ? PrismInstance.highlight(codeText, grammar, normalized)
                              : null
                                  ...
                                  {highlighted && domPurifyRef.current ? (
                                    <code
                                      dangerouslySetInnerHTML={{
                                        __html: domPurifyRef.current.sanitize(highlighted),
                                      }}
                                    />
                                  ) : (
                                    <code>{codeText}</code>
                  ...
                  <div className="message-actions">
                    <button
                      ...
                      title={
                        speakingId === index
                          ? t('chat.messages.speech_stop')
                          : t('chat.messages.speech_play')
                      }
                      aria-label={
                        speakingId === index
                          ? t('chat.messages.speech_stop')
          ...
          {isTyping && (
            <m.div className="message ai" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              <div className="message-avatar">
