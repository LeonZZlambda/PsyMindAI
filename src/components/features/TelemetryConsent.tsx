...
const TelemetryConsent: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const hasChosen =
      typeof window !== 'undefined' && localStorage.getItem('psymind_telemetry_optin') !== null
    ...
    if (!hasChosen) {
      ...
      const events = ['scroll', 'click', 'keydown', 'touchstart', 'mousemove']
      events.forEach((e) =>
        document.addEventListener(e, handleUserInteraction, { passive: true, once: true }),
      )
      ...
      const handleKeyDown = (e: KeyboardEvent) => {
        ...
        if (e.key === 'Tab') {
          ...
          const nodes = containerRef.current
            ? containerRef.current.querySelectorAll('a, button, [tabindex]:not([tabindex="-1"])')
            : []
          if (!nodes || nodes.length === 0) return
          const focusable = Array.prototype.slice
            .call(nodes)
            .filter(
              (n) => !(n as any).disabled && n.getAttribute('aria-hidden') !== 'true',
            ) as HTMLElement[]
      ...
      <div className="telemetry-consent-content">
        ...
        <p id="telemetry-desc">
          <Trans i18nKey="telemetry.description">
            📊 Dados anônimos de uso ajudam a melhorar o PsyMind.AI.
            <br />
            <br />
            🔒 Nenhuma conversa ou informação pessoal é coletada.
            <br />
        ...
        <p id="telemetry-warning" className="disclaimer" role="alert" aria-live="polite">
          <Trans i18nKey="telemetry.warning">
            <strong>Aviso:</strong> A IA oferece apoio educativo e emocional básico, mas não
            substitui o acompanhamento de um psicólogo ou profissional de saúde mental.
          </Trans>
        </p>
        <div className="telemetry-consent-actions">
          <button ref={acceptRef} className="btn-accept" onClick={handleAccept}>
            {t('telemetry.accept')}
          </button>
          ...
          <a
            ...
            aria-label={t('telemetry.more_info_aria') || t('telemetry.more_info')}
          >
            {t('telemetry.more_info')}
          </a>
        </div>
      </div>
    </div>
