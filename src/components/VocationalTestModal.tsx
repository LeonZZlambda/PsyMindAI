...
const QUESTIONS: Question[] = [
  { id: 1, text: "Você gosta de montar ou consertar objetos mecânicos no dia à dia ou de vez em quando?", dimension: 'R' },
  { id: 2, text: "Você curte investigar problemas científicos ou matemáticos para achar soluções?", dimension: 'I' },
  { id: 3, text: "Você se sente bem expressando ideias com design, artes ou criação visual?", dimension: 'A' },
  { id: 4, text: "Você gosta de ajudar pessoas a aprender algo novo ou se desenvolver?", dimension: 'S' },
  { id: 5, text: "Você se anima em liderar equipes e tomar decisões em projetos?", dimension: 'E' },
  { id: 6, text: "Você gosta de organizar dados, arquivos e seguir rotinas claras?", dimension: 'C' },
  { id: 7, text: "Você gosta de mexer com ferramentas, máquinas ou atividades práticas?", dimension: 'R' },
  { id: 8, text: "Você se sente bem observando detalhes e testando hipóteses?", dimension: 'I' },
  { id: 9, text: "Você gosta de criar algo original, mesmo que seja so por hobby?", dimension: 'A' },
  ...
  { id: 12, text: "Você se sente bem seguindo processos claros e checando tudo com cuidado?", dimension: 'C' },
  { id: 13, text: "Você gosta de atividades ao ar livre ou que envolvam uso do corpo?", dimension: 'R' },
  { id: 14, text: "Você gosta de aprender por conta propria e explorar assuntos a fundo?", dimension: 'I' },
  { id: 15, text: "Você se anima em contar histórias, escrever ou compôr algo?", dimension: 'A' },
  { id: 16, text: "Você gosta de trabalhar em equipe e criar um clima de colaboração?", dimension: 'S' },
  { id: 17, text: "Você gosta de assumir a frente para organizar pessoas e metas?", dimension: 'E' },
  { id: 18, text: "Você curte planilhas, organizacao financeira ou tarefas administrativas?", dimension: 'C' },
];
...
const buildFallbackResult = (scores: Record<Dimension, number>): VocationalResult => {
  ...
  const softSkills = Array.from(new Set(
    topDimensions.flatMap((dimension) => DIMENSION_FALLBACK[dimension].skills)
  ));
...
const VocationalTestModal: React.FC<{ isOpen: boolean; onClose: () => void }> = ({ isOpen, onClose }) => {
    ...
    try {
      ...
      const rankedDimensions = (Object.keys(scores) as Dimension[])
        .map((dimension) => ({
          ...
          percent: maxScores[dimension] ? Math.round((scores[dimension] / maxScores[dimension]) * 100) : 0
        }))
        .sort((a, b) => b.score - a.score);
      ...
      const scoresSummary = rankedDimensions.map((item) => {
      ...
      const topLabels = rankedDimensions.slice(0, 3).map((item) => t(`exams.vocational.dimensions.${item.dimension}`)).join(', ');

      const targetLanguage = i18n.language.startsWith('pt') ? 'Português' : 'English';

      const systemPrompt = `Atue como um Psicologo Organizacional especialista em carreira.\n` +
      ...
      const prompt = `Use o contexto abaixo para gerar um resumo vocacional.\n\n` +
        ...
        <AnimatePresence mode="wait">
          {!result && !isProcessing ? (
            <motion.div 
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
                  {currentStep > 0 ? (
                    ...
                    >
                      ...
                      <span style={{ fontSize: '0.9rem', fontWeight: 500 }}>{t('common.back', 'Voltar')}</span>
                    </button>
                  ) : <div />}
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="vocational-results">
                ...
                <div className="result-sections">
                  ...
                  <button 
                    ...
                    onClick={() => {
                      setInput(t("exams.vocational.result.chat_input", { area: result.topAreas[0].area }));
                      onClose();
                    }}
                  >
